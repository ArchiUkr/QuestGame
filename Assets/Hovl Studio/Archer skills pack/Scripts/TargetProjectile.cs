...
public class TargetProjectile : MonoBehaviour
{
    ...
    void OnCollisionEnter(Collision col)
    {
        if (col.gameObject.name != "Witch")
        {
            ...
            var hitInstance = Instantiate(hit, transform.position, hitRotation);
    ...
    void Update()
    {
        ...
        if (timerFire <= 0f)
        {
            Collider colFire = GetComponent<Collider>();
            colFire.GetComponent<Collider>();
